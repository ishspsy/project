...
performance1=cell(length(c_set),length(kn_set),length(rho_set));

for ii=1:length(c_set)
    for jj=1:length(kn_set);
        for kk=1:length(rho_set);
           c=c_set(ii);   kn=kn_set(jj);  rho= rho_set(kk);   
[P2] = clus_fin_update_k(rho, lam, lam2, 1, c, in_X, true_labs,kn);
...
save('robust_E_mcase2.mat', 'performance_set','missing_rate')
