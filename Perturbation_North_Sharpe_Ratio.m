function [al,de,monthly_return_perturbed_portfolio,monthly_return_benchmark]=Perturbation_North_Sharpe_Ratio(Daily_Return,Monthly_Return,al_list,c_i_list)
...
for t=time_points(1:T)
    ...
    parfor tt=1:10; % 10-fold cross validation
        ...
        for i=1:length(al_list)
            for j=1:length(c_i_list)
                ...
                c_i_temp=c_i_list(j);
                al_temp=al_list(i);
                de1=Optimization_North(de_prime,al_temp,c_i_temp,mu_training,Si_training); 
               ...
               % end
                ...
                if sum(isnan(de1)) + (sum(de1)+al_temp > 1.001) + (sum(de1)+al_temp < 0.999) > 0.5
                    ...
                    score0(i,j)=mean(r_test*w)/std(r_test*w); % Performance measure: Sharpe Ratio
                end 
            end
        end
        CV_Score(:,:,tt)=score0; 
    ...
    de1=Optimization_North(de_prime,al_temp,c_i_temp,mu,Si); 
	%while sum(isnan(de1))>0
	while sum(isnan(de1)) + (sum(de1)+al_temp > 1.001) + (sum(de1)+al_temp < 0.999) >0.5 
        c_i_temp=c_i_temp-0.05;
        de1=Optimization_North(de_prime,al(time_index),c_i_temp,mu,Si);
	end
